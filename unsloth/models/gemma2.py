...
def fast_rms_layernorm_gemma2_compiled(layernorm, X, gemma=True):
    ...
    X = (
        X
        * torch.rsqrt(X.square().mean(-1, keepdim=True) + layernorm.eps)
...
def Gemma2Model_fast_forward_inference(
    ...
    for idx, decoder_layer in enumerate(self.model.layers):
        ...
        residual = hidden_states
        hidden_states = fast_rms_layernorm_inference_gemma(
            decoder_layer.pre_feedforward_layernorm, hidden_states, out_weight
        )
        hidden_states = fast_geglu_inference(decoder_layer.mlp, hidden_states)
        hidden_states = fast_rms_layernorm_inference_gemma(
            decoder_layer.post_feedforward_layernorm, hidden_states, out_weight
        )
        hidden_states += residual
