    ...
    if do_prefill:
        ...
        if attention_size != self.hidden_size:
            self.temp_O = torch.empty(
                (1, bsz, self.hidden_size), dtype=dtype, device="cuda:0"
            )
        else:
        ...
        if isinstance(model, PeftModelForCausalLM):
            ...
            if check_all:
                ...
                if "embed_tokens" in new_target_modules:
                    ...
                    dtype = (
                        model.model.model.embed_tokens.modules_to_save.default.weight.dtype
                    )
                    model.model.model.embed_tokens.modules_to_save.default.to(
                        device="cuda:0",
                        dtype=(dtype if (dtype != torch.float16) else torch.float32),
                        non_blocking=True,
                    )
                    ...
                    model.model.model.embed_tokens.original_module.to(
                ...
                if "lm_head" in new_target_modules:
                    ...
                    dtype = (
                        model.model.model.lm_head.modules_to_save.default.weight.dtype
                    )
                    ...
                    model.model.lm_head.original_module.to(
                        device="cpu", non_blocking=True
                    )
