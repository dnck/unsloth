...
if (
    hasattr(transformers.cache_utils, "DynamicCache")
    and transformers.cache_utils.DynamicCache.__getitem__.__name__
    != "__cache_utils_getitem__"
):
...
if Version(xformers_version) >= Version("0.0.27"):
    ...
    if (
        hasattr(accelerate.utils.operations, "send_to_device")
        and accelerate.utils.operations.send_to_device.__name__
...
prepare = prepare.replace(
    x, f"self.state.distributed_type = DistributedType.NO\n{s}{x}", 1
)
    ...
    if len(batch_samples) > 0 and "labels" in batch_samples[0]:
        try:
            num_items_in_batch = sum(
                [
                    torch.count_nonzero(x["labels"][..., 1:] != -100)
