...
def GraniteAttention_fast_forward(
    ...
    **kwargs,
    ...
    if not HAS_FLASH_ATTENTION and attention_mask is None:
        ...
        A = flash_attn_func(
            Q, K, V, causal=True, window_size=window, softmax_scale=self.scaling
        )
    else:
    ...
    **kwargs,
    ...
    if use_cache and hasattr(
        ...
        hidden_states = torch.add(
            residual, hidden_states, alpha=self.config.residual_multiplier
        )
...
def GraniteModel_fast_forward_inference(
    ...
    position_embeddings = self.model.rotary_emb(
        hidden_states, position_ids, self.max_seq_length
    )
