...
def unsloth_save_model(
    ...
    if type(max_shard_size) is str:
        gb_found = re.match(
            "([0-9]{1,})[\s]{0,}GB", max_shard_size, flags=re.IGNORECASE
        )
        mb_found = re.match(
            "([0-9]{1,})[\s]{0,}MB", max_shard_size, flags=re.IGNORECASE
        )
        if gb_found:
...
def install_llama_cpp_old(version=-10):
    ...
    # Check if successful
    if not os.path.exists("llama.cpp/quantize") and not os.path.exists(
...
def save_to_gguf(
    ...
    llama_quantize_executable: str = None,
    convert_hf_to_gguf_py: str = None,
):

    required_executables = ["llama-quantize", "convert_hf_to_gguf.py"]
    if llama_quantize_executable is None or convert_hf_to_gguf_py is None:
        print(
            f"ERROR: Please install llama.cpp and pass the path to the required executables={required_executables}"
        )
        return

    known_model_dtypes = {"float16": "f16", "bfloat16": "bf16"}
    if model_dtype not in known_model_dtypes.keys():
        print(
            f"ERROR: Unknown model dtypes. Given {model_dtype} but only know {known_model_dtypes}"
        )
        return
    model_dtype = known_model_dtypes[model_dtype]
    ...
    if not is_sentencepiece:
        use_fast_convert = False  # Llama-3
    ...
    print(
        """INFO: **Conversion from QLoRA to GGUF wait times**
INFO: Install llama.cpp. Estimated wait time: 3m
INFO: Convert HF to GGUF 16bits. Estimated wait time: 3m
INFO: Convert GGUF 16bits to {}. Estimated wait timee: 10m / each""".format(
            quantization_method
        )
    )
    ...
    if first_conversion == "f16":
        pass
    elif first_conversion == "bf16":
        pass
    elif first_conversion == "f32":
        pass
    ...
    if first_conversion == "f32":
        strength = 3
    elif first_conversion == "f16":
        strength = 2
    elif first_conversion == "bf16":
        strength = 1
    elif first_conversion == "q8_0":
    ...
    if not use_fast_convert and (
        first_conversion != "f16"
        or first_conversion != "bf16"
        or first_conversion != "f32"
    ):
        pass
    ...
    model_name_gguf_filepath = str(
        (
            Path(model_directory) / f"{model_name}_{first_conversion.upper()}.gguf"
        ).absolute()
    )

    print(
        f"""INFO: Converting model {model_directory}/{first_conversion} to GGUF
INFO: Expected output path: {model_name_gguf_filepath}
INFO: Estimated wait time is 3m"""
    )
    ...
    command = f"{convert_hf_to_gguf_py} {model_directory} --outfile {model_name_gguf_filepath} --outtype {first_conversion}"
    ...
    if not os.path.isfile(model_name_gguf_filepath):
        if IS_KAGGLE_ENVIRONMENT:
            if (
                not Path(model_name_gguf_filepath)
                .resolve()
                .is_relative_to(Path("/tmp").resolve())
            ):
                raise RuntimeError(
                    f"ERROR: Quantization failed for {model_name_gguf_filepath}\n"
    ...
    print(f"INFO: Conversion complete! Output location: {model_name_gguf_filepath}")

    full_precision_location = model_name_gguf_filepath

    all_saved_locations = [
        full_precision_location,
    ]
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            print(
                f"INFO: Converting GGUF 16bit to {quant_method}. Expected wait time > 20m"
            )

            model_name_gguf_filepath = str(
                (
                    Path(model_directory) / f"{model_name}_{quant_method.upper()}.gguf"
                ).absolute()
            )

            command = f"{llama_quantize_executable} {full_precision_location} {model_name_gguf_filepath} {quant_method} {n_cpus}"
            ...
            print(
                f"Unsloth: Conversion complete! Output location: {model_name_gguf_filepath}"
            )

            all_saved_locations.append(model_name_gguf_filepath)
...
def unsloth_save_pretrained_gguf(
    ...
    if push_to_hub:
        ...
        for file_location in all_file_locations:
            ...
            link = (
                f"{username}/{new_save_directory.lstrip('/.')}"
                if username not in new_save_directory
                else new_save_directory.lstrip("/.")
            )
